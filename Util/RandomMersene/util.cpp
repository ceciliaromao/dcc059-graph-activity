...
/***********************************************************************************
 * Funcao    : unsigned   int xrandom_range (unsigned   int min, unsigned   int max)                 *
 * Criacao   : 17/04/2002                                                          *
...
//2022/12/28 -> Lucas Marins 
inline double xrandomRanged (double min, double max)
{
#ifdef MTWISTER
  double delta;
  ...
  delta = max - min;
  if (delta > 0)
    return  min + doubleRandom (delta + 1);
...
/***********************************************************************************
 * Funcao    : unsigned   int xrandom_exclude (unsigned   int excluded, unsigned   int limit)        *
 * Criacao   : 17/04/2002                                                          *
...
void testaRandomMersene(unsigned int num, int vet[], int n){
  int k;
  //cout << "[ " ;
  for(unsigned int i=0; i<(num*n); i++){
    k = xrandomRange(0,num-1);
    //cout << k << " "  /*intRandom(num) <<*/;
    vet[k]=vet[k]+1;
  }
  //cout << " ]" << endl;
}

void testaRandomRand(unsigned int num, int vet[], int n){
  int k;
  //cout << "[ " ;
  for(unsigned int i=0; i<(num*n); i++){
    k = rand()%num;
...
int main(){
    ...
    testaRandomMersene(TAM, freqVet,MULT);

    cout << "Rand[ " ;
    
    for (i=0; i<TAM; i++){
       //cout << abs((freqVet[i]/(float)MULT-1) << " " ;
       delta+=abs((freqVet[i]/(float)MULT)-1);
    }
    cout << " ] Delta Rand = " << abs((delta/TAM*100.0)-1) << endl;
    cout << endl;
   ...
   for (i=0; i<TAM; i++){
    ...
    srand(time(NULL));
    testaRandomRand(TAM, freqVet,MULT);
    delta =0;
    cout << "Mersene[ " ;
    for (i=0; i<TAM; i++){
       //cout << abs((freqVet[i]/(float)MULT)-1) << " ";
    ...
    cout << " ] Delta Mersene= " << abs((delta/TAM*100.0)-1) << endl;}
